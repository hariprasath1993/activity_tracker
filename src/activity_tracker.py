...
with st.container():
    ...
    if activity_type == 'Call':
        ...
        with st.form(key="call activity"):
            ...
            if call_submit:
                if not Utils.check_for_time_validity():
                    st.error(
                        "You cannot log activities for dates more than 1 day in the past.")
                else:
                    if calling_activity.is_successful:
        ...
        scheduled_meeting.members = selected_user_ids
        with st.form(key="scheduled_meeting_activity"):
            ...
            if scheduled_meeting_submit:
                ...
                if len(scheduled_meeting.meeting_minutes.split()) < 2:
                    st.error(
                        "Insufficient words to create a record. Please write at least 300 words.")
                elif scheduled_meeting.action_item.strip() == "":
                    st.error(
                        "Action Items cannot be empty. Please provide at least one action item.")
                else:
                    st.session_state.meeting_submitted = True
